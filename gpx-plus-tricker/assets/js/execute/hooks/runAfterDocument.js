...
function berryFeeder(fnames, src, srcName, newWindow) {
    ...
    if (localStorage.berryFeederSrc) {
        input("You may already have a Berry Feeder session open!  Opening another one can interfere with the first one.  Would you like to proceed?",
            function (a) {
                if (a) {
                    proceed();
                }
...
function modifyUsersScript() {
    const buttonUsersOpen = $("#usersOpen");

    // modify a part of users.js script of the site
    // to allow us to open above 300 pokemons for interaction
    buttonUsersOpen.off("submit");

    buttonUsersOpen.submit(function (e) {
        e.preventDefault();
